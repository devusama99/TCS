...
// FA18-BCS-071 & FA18-BCS-073 ASSIGNEND ADMIN ROUTE:
// UPDATING INFORMATION AGAIST THE CLASS ID (NAME)

router.put('/class:id',function(req, res, next){
    Class.findOneAndUpdate({'_id': req.params.id}, req.body).then((updateClassName)=>{
        console.log(updateClassName);
        console.log("Class Name Updated Successfully. ");
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json');
        res.json(updateClassName);
    }), (err) => next(err);
});
